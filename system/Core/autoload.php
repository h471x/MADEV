 ...
 * App   fait référence aux classes de l’application de l’utilisateur (situées dans le dossier app).
 *
 * C'est-à-dire que toutes les classes doivent appartenir aux namespaces MADEV ou App.
 */
const ALIASES = [
    'MADEV' => 'system',
    'App' => 'app'
 ...
 */ function ($class) {
    ...
    if (count($namespaceParts) === 1)
         throw new Exception("La classe \"$class\" doit appartenir aux namespaces \"MADEV\" ou \"App\"");

    if (in_array($namespaceParts[0], array_keys(ALIASES))) $namespaceParts[0] = ALIASES[$namespaceParts[0]];
    else throw new Exception("Namespace \"$namespaceParts[0]\" invalide. Un namespace doit commencer par : \"MADEV\" ou \"App\"");

    /*
     * Si une classe se situe dans le namespace App\Models\Entity,
